...
// ✅ YOUR SPECIFIC KEY IS NOW HARDCODED HERE
...
export default function Chatbot() {
  ...
  const handleSend = async (textInput = input) => {
    ...
    try {
        ...
        console.log("Talking to Gemini..."); // Debug log
        const genAI = new GoogleGenerativeAI(API_KEY);
        const model = genAI.getGenerativeModel({ model: "gemini-2.5 flash" });
        
        const prompt = `You are Vyom, an expert tour guide for Hampi. 
        User asking: "${messageText}". 
        Keep your answer short (max 60 words), friendly, and exciting.`;
                            ...
                            <div className="space-y-4">
                                {messages.map((msg, i) => (
                                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[85%] p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-brand-accent text-white rounded-br-none' : 'bg-white border border-gray-200 text-brand-dark rounded-bl-none'}`}>
                                            {msg.text}
                                        </div>
                                    </div>
                                ))}
