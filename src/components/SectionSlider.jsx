...
export default function SectionSlider({ title, items, isActivity = false, onItemClick, icon: Icon, theme = 'orange' }) {
    ...
    const Card = ({ item }) => (
        <div
            ...
            className={`min-w-[240px] bg-[#FAF3E1] rounded-2xl p-3 shadow-lg flex-shrink-0 snap-center border border-brand-dark/5 hover:scale-[1.02] transition-transform duration-300 flex flex-col justify-between cursor-pointer group`}
        >
            <div>
                {/* Image Section */}
    ...
    return (
        <div className="mt-8 pl-6 relative z-10 hidden-scrollbar">
            ...
            {showAll ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pb-8 pr-6">
